...
function paintStatistics(){
 ...
 document.getElementById("content").innerHTML=`
  ...
  <div class="stats-kpis">
   ...
   ${statsKpi("Centros a cotización",k.quoted_centers,"Estado general o algún viaje a cotización")}
   ${statsKpi("Conversión a presupuesto",k.quote_conversion_pct,"Centros a cotización / trabajados","success",true)}
  ...
  <div class="stats-grid">
   ...
   <div class="card stats-panel"><h2>Cartera por zona</h2><div class="panel-sub">Distribución actual del alcance y avance de trabajo.</div>${statsBarRows(zones,"community","portfolio_total")}${zones.length?`<div class="stats-table-wrap mt14"><table><thead><tr><th>Zona</th><th>Cartera</th><th>Trabajados</th><th>Interesados</th><th>Centros a cotización</th><th>Vencidos</th></tr></thead><tbody>${zones.map(z=>`<tr><td>${esc(z.community)}</td><td>${statsMetric(z.portfolio_total)}</td><td>${statsMetric(z.worked_centers)}</td><td>${statsMetric(z.interested_centers)}</td><td>${statsMetric(z.quoted_centers)}</td><td class="${Number(z.overdue_followups)?"due":""}">${statsMetric(z.overdue_followups)}</td></tr>`).join("")}</tbody></table></div>`:""}</div>
  ...
  <details class="card stats-definitions"><summary>Cómo se calculan estos indicadores</summary><ul><li><b>Estado efectivo del centro:</b> si tiene viajes activos, se prioriza A cotización, después Interesado, todos No interesado y, en último lugar, Pendiente. El colegio cuenta una sola vez aunque tenga varios viajes.</li><li><b>Cartera trabajada:</b> centro con al menos un contacto general o de viaje, o con estado efectivo distinto de Pendiente.</li><li><b>Viaje trabajado:</b> grupo con al menos un contacto asociado o con estado distinto de Pendiente.</li><li><b>Conversión por viaje:</b> viajes A cotización divididos entre viajes trabajados; un colegio puede aportar varios viajes.</li><li><b>Seguimiento a tiempo:</b> compromisos vencidos del periodo cuyo siguiente contacto quedó registrado antes o en la fecha y hora prevista. Los incumplidos o no atendidos reducen el porcentaje.</li><li><b>Nuevos centros:</b> sólo altas manuales creadas en el periodo; no cuenta la importación inicial.</li><li><b>Foto actual frente a periodo:</b> cartera, estados, agenda y oportunidades son actuales; contactos, altas y puntualidad usan el periodo elegido.</li></ul></details>
