...
function dialogHtml(c){
 ...
 const contactEventsCount=events.filter(e=>e.entryType!=="internal_note").length,internalNotesCount=events.length-contactEventsCount;
 const activeOpportunities=(c.workspace?.opportunities||[]).filter(o=>o.active),generalFollowupKey=c.nextContact?`${c.nextContact}T${c.nextContactTime||"09:00"}`:"",hasOverdueGeneralFollowup=!!(generalFollowupKey&&generalFollowupKey<=nowKey()&&!isClosedStatus(c));
 ...
 <div class="modal-body">
  ...
  <div class="modal-section"><h3>Registrar contacto o anotación interna</h3><p class="section-note">El contacto comercial mantiene canal, resultado y agenda. La anotación interna guarda únicamente el texto, aparece en este historial y no cuenta como contacto ni modifica estadísticas.</p>
   <form id="contactForm" class="form-grid" data-dirty-scope="contact-event">
    ...
    <div class="field"><label>Próximo contacto</label><input type="date" id="cNext"><span class="field-help" id="cNextHelp" aria-live="polite">Indica una nueva fecha solo cuando corresponda continuar el seguimiento.</span></div>
    <div class="field"><label>Hora próximo contacto</label><input type="time" id="cNextTime"></div>
    <div class="opportunity-selector"><label>¿A qué viaje(s) corresponde?</label><div class="field-help">Puedes marcar varios si la misma conversación afecta a varios grupos. El resultado y la próxima fecha se aplicarán a todos los seleccionados. Si son distintos, registra cada resultado por separado.</div><div class="opportunity-checks">${activeOpportunities.map(o=>`<label class="opportunity-check"><input type="checkbox" name="cOpportunity" value="${esc(o.opportunity_id)}"><span><b>${esc(o.cycle)}${o.destination?` · ${esc(o.destination)}`:""}</b><small>${esc(o.opportunity_id)} · ${esc(o.status)}</small></span></label>`).join("")||'<span class="muted">No hay viajes activos. El contacto se registrará como seguimiento general del centro.</span>'}</div>${hasOverdueGeneralFollowup&&activeOpportunities.length?`<div class="contact-general-scope"><label><input type="checkbox" id="cResolveGeneral" disabled> Esta misma gestión también atiende el seguimiento general vencido del ${esc(fmtNext(c))}</label><div class="field-help" id="cResolveGeneralHelp" aria-live="polite">Selecciona primero al menos un viaje para poder aplicar la misma gestión a ambos ámbitos.</div></div>`:""}</div>
    <div class="field span4"><label>Qué se ha hablado / acordado · o anotación interna</label><textarea id="cNote" maxlength="4000" placeholder="Ej.: Hablamos con Javier, jefe de estudios. Organizan viaje de 4º ESO para unos 55 alumnos y quiere recibir propuestas de Italia…" required></textarea></div>
    <div class="field span4 contact-actions"><button class="btn primary" type="submit">Registrar contacto</button><button class="btn" type="button" id="internalNoteBtn">Guardar anotación interna</button></div>
   </form>
  </div>
  <div class="modal-section"><h3>Historial de actividad (${contactEventsCount} contacto${contactEventsCount===1?"":"s"} · ${internalNotesCount} anotación${internalNotesCount===1?"":"es"} interna${internalNotesCount===1?"":"s"})</h3><div class="history-note"><span>Ordenado del más reciente al más antiguo. Contactos y anotaciones conviven cronológicamente, pero se contabilizan por separado.</span><b>Historial acumulativo</b></div><div class="timeline">
   ${events.length?events.map((e,index)=>{const internal=e.entryType==="internal_note";return `<div class="event ${internal?"internal-note":""}"><div class="event-head"><span><span class="history-sequence">${events.length-index}</span><strong>${esc(e.author)}</strong> · ${internal?'<span class="event-tag internal-note">Anotación interna</span>':`Contacto ${esc(fmtDateTime(e.contactedAt)||fmtDate(e.date))} <span class="event-tag">${esc(e.channel)}</span><span class="event-tag">${esc(e.result)}</span>`}</span><span>Registrado ${fmtDateTime(e.createdAt)}</span></div>${internal?'<div class="muted mt6">No cuenta como contacto ni modifica la agenda o las estadísticas.</div>':`${e.contactName?`<div class="muted mt6">Persona: ${esc(e.contactName)}${e.contactRole?` · ${esc(e.contactRole)}`:""}</div>`:""}${e.opportunities?.length?`<div class="event-links">${e.opportunities.map(o=>`<span class="event-tag">${esc(o.cycle)}${o.destination?` · ${esc(o.destination)}`:""} · ${esc(o.opportunity_id)}</span>`).join("")}${e.alsoResolvedGeneralFollowup?'<span class="event-tag scope-confirmed">También atendió la agenda general</span>':""}</div>`:'<div class="muted mt6">Seguimiento general del centro</div>'}`}<div class="event-note">${esc(e.note)}${!internal&&e.nextContact?`<div class="muted mt6">Próximo contacto: ${fmtDate(e.nextContact)}${e.nextContactTime?` · ${esc(e.nextContactTime)}`:""}</div>`:""}</div></div>`}).join(""):'<div class="empty">Aún no hay contactos ni anotaciones registrados con este centro.</div>'}
  </div></div>
 </div>`;
}
...
function bindCenterDialog(c){
 ...
 const internalNoteBtn=dlg.querySelector("#internalNoteBtn");if(internalNoteBtn){internalNoteBtn.onclick=addInternalNote;if(contactEventInFlight){internalNoteBtn.disabled=true;internalNoteBtn.textContent="Guardando…"}}
 const contactResult=dlg.querySelector("#cResult");if(contactResult)contactResult.addEventListener("change",syncContactFollowupControls);
 dlg.querySelectorAll('input[name="cOpportunity"]').forEach(input=>input.addEventListener("change",syncContactScopeControls));
 syncContactFollowupControls();syncContactScopeControls();
