...
function syncContactFollowupControls(){
 const result=document.getElementById("cResult")?.value||"",date=document.getElementById("cNext"),time=document.getElementById("cNextTime"),help=document.getElementById("cNextHelp"),closes=["Pide presupuesto","No interesado"].includes(result);if(!date||!time)return;
 if(closes){date.value="";time.value=""}date.disabled=closes;time.disabled=closes;
 if(help)help.textContent=closes?`«${result}» cierra este seguimiento. La próxima fecha queda vacía.`:"Indica una nueva fecha solo cuando corresponda continuar el seguimiento.";
}
function syncContactScopeControls(){
 const selected=document.querySelectorAll('input[name="cOpportunity"]:checked').length,resolve=document.getElementById("cResolveGeneral"),help=document.getElementById("cResolveGeneralHelp");if(!resolve)return;
 resolve.disabled=selected===0;if(!selected)resolve.checked=false;
 if(help)help.textContent=selected?"Márcalo solo si esta misma conversación atendió también el recordatorio general indicado.":"Selecciona primero al menos un viaje para poder aplicar la misma gestión a ambos ámbitos.";
}
async function addContact(){
 ...
 try{
  ...
  if(!opportunityIds.length||alsoResolveGeneral){c.status=RESULT_STATUS[result]||c.status;if(isClosedStatus(c)){c.nextContact="";c.nextContactTime=""}else if(next){c.nextContact=next;c.nextContactTime=nextTime}else{c.nextContact="";c.nextContactTime=""}}
