-- R10 hotfix: make the scope of a contact explicit when it is linked to one
-- or more travel opportunities. A linked contact can optionally resolve the
-- already-overdue general follow-up without creating a duplicate event.

alter table public.contact_events
  add column if not exists also_resolved_general_followup boolean not null default false;

comment on column public.contact_events.also_resolved_general_followup is
  'True only when a contact linked to one or more trips also updated the overdue general follow-up of the center.';

-- Replace the RPC atomically with a backwards-compatible optional argument.
-- Existing clients can omit p_also_resolve_general_followup and retain the
-- previous behavior.
...
create function public.register_contact_multi_v1(
  ...
  p_also_resolve_general_followup boolean default false
) returns jsonb
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  v_next timestamptz;
  v_linked boolean;
  v_also_resolve_general boolean := coalesce(p_also_resolve_general_followup, false);
...
begin
  ...
  if v_also_resolve_general and not v_linked then
    raise exception 'GENERAL_FOLLOWUP_SCOPE_REQUIRES_TRIP';
  end if;
  if v_also_resolve_general
     and (v_state.next_contact_at is null or v_state.next_contact_at > now()) then
    raise exception 'GENERAL_FOLLOWUP_NOT_OVERDUE';
  end if;
  ...
  set status = case
        when v_linked and not v_also_resolve_general then s.status
        else v_status
      end,
      next_contact_at = case
        when v_linked and not v_also_resolve_general then s.next_contact_at
        else v_next
      end,
      last_contact_at = p_contacted_at,
      last_result = p_result,
...
comment on function public.register_contact_multi_v1(
  text, timestamptz, text, text, text, timestamptz, bigint, bigint, text[], jsonb, boolean
) is 'Registra una conversación y la vincula atómicamente a ninguno, uno o varios viajes; puede actualizar también un seguimiento general ya vencido de forma explícita.';
